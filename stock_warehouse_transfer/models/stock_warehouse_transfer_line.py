...
class StockWarehouseTransferLine(models.Model):
    ...
    def product_id_change(self):
        for rec in self:
            if rec.product_id:
                rec.product_uom_id = (rec.product_id.uom_id and self.product_id.uom_id.id or False)
                self._get_qty_avble()
            else:
    ...
    def _get_transfer_locations(self):
        for rec in self:
            ...
            for location in transit_locations:
                if location.warehouse_id == rec.transfer.dest_warehouse:
    ...
    def get_move_vals(self, picking, group):
        ...
        product_uom_qty, product_uom = self.product_uom_id._adjust_uom_quantities(self.product_qty, self.product_id.uom_id)
        return {
